...
Circle::Circle(double cx0, double cy0, double r0, double m0, double dx0, double dy0, double v0, ALLEGRO_COLOR col):
    cx(cx0),
    cy(cy0),
    r(r0),
    m(m0),
...
bool Circle::Overlap(Circle cir) {
    ...
    return true;
    */
    return ((cir.cx - cx) * (cir.cx - cx) + (cir.cy - cy) * (cir.cy - cy)) < (cir.r + r) * (cir.r + r);
}
...
void Circle::Random(double cxmin, double cxmax, double cymin, double cymax, double mmin, double mmax, double dmin, double dmax, double vmin, double vmax)
{//to not get outside the screen ,cxmin and cymin should be >=rmax, and cxmax and cymax should be <=(SCREEN_W or SCREEN_H)-rmax
    m  = mmin + rand() % (long)(mmax - mmin) + (double)rand() / (RAND_MAX - 1);
    r = m*15;
